function y = pvoc(x, alpha, n)
% y = pvoc(x, r, n)  Time-scale a signal to alpha times longer with phase vocoder
%      x is an input sound. n is the FFT size, defaults to 1024.  
%      Calculate the 25%-overlapped STFT, squeeze it by a factor of 1/alpha, 
%      inverse spegram.
...
X = scf * stft(x', n, n, hop/2);

% Calculate the new timebase samples
[rows, cols] = size(X);
t = 0:(1.0/alpha):(cols-2);
